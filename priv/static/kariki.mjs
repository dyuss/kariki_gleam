...
function append(first, second) {
  return append_loop(reverse2(first), second);
}
...
function index3(loop$path, loop$position, loop$inner, loop$data, loop$handle_miss) {
  while (true) {
    ...
    if (path instanceof Empty) {
      ...
      return push_path(_pipe, reverse2(position));
    } else {
      let key = path.head;
...
function target(value2) {
  return attribute2("target", value2);
}
function rel(value2) {
...
function do_extract_keyed_children(loop$key_children_pairs, loop$keyed_children, loop$children) {
  while (true) {
    ...
    if (key_children_pairs instanceof Empty) {
      return [keyed_children, reverse2(children)];
    } else {
...
function tbody(attributes, children) {
  return element3("tbody", attributes, children);
}
...
var FILEPATH = "src/kariki.gleam";
var Loading = class extends CustomType {
};
var AppError = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
var AppData = class extends CustomType {
  constructor(db, visible_games, sorting, filters) {
    super();
    this.db = db;
    this.visible_games = visible_games;
...
var UserChangedSorting = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
};
var UserChangedTitleFilter = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
};
var UserChangedStatusFilter = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
};
var UserChangedConditionFilter = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
function prepare_visible_games(games, sorting, filters) {
  ...
  return sort_games(_pipe$1, sorting);
}
function update2(model, msg) {
  if (msg instanceof AppLoadedDbJson) {
    let db = msg[0];
    let _block;
    if (db instanceof Ok) {
      let db$1 = db[0];
      let next_sorting = new SortById();
      let next_filters = new Filters(
        "",
        toList([]),
        (() => {
          let _pipe = db$1.games;
          let _pipe$1 = map(_pipe, (game) => {
            return game.status;
          });
      ...
      _block = new Loaded(
        new AppData(
          db$1,
          prepare_visible_games(db$1.games, next_sorting, next_filters),
          next_sorting,
          next_filters
        )
      );
      ...
      let error = db[0];
      _block = new AppError(error);
    }
    let model$1 = _block;
    return [model$1, none()];
  } else if (msg instanceof UserChangedSorting) {
    let sorting = msg[0];
    let _block;
    if (model instanceof Loaded) {
      let data = model[0];
      _block = new Loaded(
        new AppData(
          data.db,
          prepare_visible_games(data.db.games, sorting, data.filters),
          sorting,
          data.filters
        )
      );
    ...
    let model$1 = _block;
    return [model$1, none()];
  } else if (msg instanceof UserChangedTitleFilter) {
    let title = msg[0];
    let _block;
    if (model instanceof Loaded) {
      let data = model[0];
      let _block$1;
      let _record = data.filters;
      _block$1 = new Filters(
        title,
        _record.statuses,
        _record.available_statuses,
        _record.conditions
      );
      let next_filters = _block$1;
      _block = new Loaded(
        new AppData(
          data.db,
          prepare_visible_games(data.db.games, data.sorting, next_filters),
          data.sorting,
          next_filters
        )
      );
    ...
    let model$1 = _block;
    return [model$1, none()];
  } else if (msg instanceof UserChangedStatusFilter) {
    let status = msg[0];
    let _block;
    if (model instanceof Loaded) {
      let data = model[0];
      let _block$1;
      let $ = contains(data.filters.statuses, status);
      if ($) {
        let _pipe = data.filters.statuses;
        _block$1 = filter(_pipe, (s) => {
          return s !== status;
        });
      } else {
        let _pipe = data.filters.statuses;
        _block$1 = append(_pipe, toList([status]));
      }
      let next_statuses = _block$1;
      let _block$2;
      let _record = data.filters;
      _block$2 = new Filters(
        _record.title,
        next_statuses,
        _record.available_statuses,
        _record.conditions
      );
      let next_filters = _block$2;
      _block = new Loaded(
        new AppData(
          data.db,
          prepare_visible_games(data.db.games, data.sorting, next_filters),
    ...
    if (model instanceof Loaded) {
      let data = model[0];
      let _block$1;
      let $ = contains(data.filters.conditions, condition);
      if ($) {
        let _pipe = data.filters.conditions;
        _block$1 = filter(
          _pipe,
          (c) => {
            return !isEqual(c, condition);
          }
        );
      } else {
        let _pipe = data.filters.conditions;
        _block$1 = append(_pipe, toList([condition]));
      }
      let next_conditions = _block$1;
      let _block$2;
      let _record = data.filters;
      _block$2 = new Filters(
        _record.title,
  ...
  return div(
    ...
    toList([
      ...
      select(
        ...
        toList([
          option(toList([value("id")]), "\u041F\u043E \u043E\u0431\u043D\u043E\u0432\u043B\u0435\u043D\u0438\u044E"),
          option(toList([value("price")]), "\u041F\u043E \u0446\u0435\u043D\u0435"),
          option(toList([value("title")]), "\u041F\u043E \u043D\u0430\u0437\u0432\u0430\u043D\u0438\u044E")
        ])
      )
    ])
  );
}
function view_title_filter(data) {
  return div(
    toList([class$("title-filter headings")]),
    toList([
      h6(toList([]), toList([text3("\u041D\u0430\u0437\u0432\u0430\u043D\u0438\u0435")])),
      input(
        toList([
          ...
          on_input(
            (var0) => {
              return new UserChangedTitleFilter(var0);
            }
          )
        ])
      )
...
function view_status_checkbox(status, statuses) {
  return label(
    toList([for$(status)]),
    toList([
      input(
        toList([
          ...
          checked(
            (() => {
              let _pipe = statuses;
              return contains(_pipe, status);
            })()
          ),
          on_change(
            (_) => {
              return new UserChangedStatusFilter(status);
            }
          )
        ])
      ),
      span(
        toList([class$("checkable")]),
        toList([text3(status)])
      )
    ])
  );
}
function view_status_filter(data) {
  return div(
    toList([class$("status-filter headings")]),
    prepend(
      h6(toList([]), toList([text3("\u0421\u0442\u0430\u0442\u0443\u0441")])),
      (() => {
        let _pipe = data.filters.available_statuses;
        return map(
          _pipe,
          (status) => {
            return view_status_checkbox(status, data.filters.statuses);
          }
        );
      })()
    )
  ...
  return label(
    ...
    toList([
      input(
        toList([
          type_("checkbox"),
          id(condition_to_string(condition)),
          checked(
            (() => {
              let _pipe = conditions;
              return contains(_pipe, condition);
            })()
          ),
          on_change(
            (_) => {
              return new UserChangedConditionFilter(condition);
            }
          )
        ])
      ),
      span(
        toList([class$("checkable")]),
        toList([text3(condition_to_string(condition))])
      )
    ])
  );
}
function view_condition_filter(data) {
  return div(
    toList([class$("condition-filter headings")]),
    prepend(
      h6(toList([]), toList([text3("\u0421\u043E\u0441\u0442\u043E\u044F\u043D\u0438\u0435")])),
      (() => {
        let _pipe = toList([new NewCondition(), new UsedCondition()]);
        return map(
          _pipe,
          (condition) => {
            return view_condition_checkbox(condition, data.filters.conditions);
          }
        );
      })()
    )
      ...
      td(
        ...
        toList([
          a(
            ...
            toList([text3(game.title)])
          )
        ])
      ),
      td(
        toList([class$("game_condition")]),
        toList([text3(condition_to_string(game.condition))])
      ),
      td(
        toList([class$("game_status")]),
        toList([text3(game.status)])
      ),
      td(
        toList([class$("game_price")]),
        toList([text3(to_string(game.price))])
      )
    ])
  );
}
function view_games_list(data) {
  return table(
    toList([role("grid")]),
    toList([
      tbody(
        ...
        map(
          data.visible_games,
          (game) => {
            return [to_string(game.id), view_games_list_item(game)];
          }
        )
      )
    ])
...
function view_list(data) {
  return div(
    ...
    toList([
      div(
        toList([class$("games-list__header")]),
        toList([view_games_count(data), view_db_date(data.db)])
      ),
...
function view(model) {
  if (model instanceof Loading) {
    return div(toList([]), toList([text3("loading...")]));
  } else if (model instanceof AppError) {
    let error = model[0];
    return div(toList([]), toList([text3(error)]));
  } else {
    let data = model[0];
    return div(
      toList([class$("main-page")]),
      toList([view_filters(data), view_list(data)])
    );
  }
}
function fetch_db2() {
